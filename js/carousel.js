...
function montrerSlides(n) {
  ...
  let points = document.getElementsByClassName("pt");
  if (n > slides.length) index_slide = 1;
  if (n < 1) index_slide = slides.length;
  for (i = 0; i < slides.length; i++) slides[i].style.display = "none";
  for (i = 0; i < points.length; i++) points[i].className = points[i].className.replace(" active", "");
